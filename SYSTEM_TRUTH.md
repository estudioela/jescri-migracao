...
7. **(Resolvido em 2026-07-06) Drive API desabilitada no projeto GCP vinculado (`jescri-migracao`, nº `607782229022`)**: `appsscript.json` declara o escopo OAuth `https://www.googleapis.com/auth/drive` (necessário pro upload de matérias — `DriveApp` + chamada direta à API de upload resumable v3), mas a API correspondente (`drive.googleapis.com`) nunca tinha sido habilitada no projeto GCP standard vinculado ao script. Isso quebrou dois fluxos: upload de matérias (dependência direta de Drive) **e** carregamento de histórico (`getHistorico()` não usa Drive diretamente — só `SpreadsheetApp` — mas a falha de autorização de um escopo declarado no manifesto pode invalidar a autorização de toda a execução do Web App, não só das funções que de fato usam Drive). Corrigido via `gcloud services enable drive.googleapis.com --project=jescri-migracao` — a conta (`elafashionmkt@gmail.com`) tinha permissão suficiente (Service Usage API já habilitada, sem política de organização bloqueando); não foi necessário mudar `appsscript.json`/escopos/código. Nenhum redeploy necessário (habilitação de API é a nível de projeto GCP, não de deployment do Web App). **Lição**: um escopo OAuth novo no manifesto exige confirmar que a API correspondente está habilitada no projeto GCP vinculado — declarar o escopo sem a API habilitada quebra a autorização de todo o script, não só da função que usa aquele escopo.
