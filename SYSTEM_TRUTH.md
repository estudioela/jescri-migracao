...
7. **(Parcialmente resolvido, PENDENTE ação manual — 2026-07-06) Drive API estava desabilitada no projeto GCP vinculado (`jescri-migracao`, nº `607782229022`)**: `appsscript.json` declara o escopo OAuth `https://www.googleapis.com/auth/drive` (necessário pro upload de matérias — `DriveApp` + chamada direta à API de upload resumable v3), mas a API correspondente (`drive.googleapis.com`) nunca tinha sido habilitada no projeto GCP standard vinculado ao script. Habilitada via `gcloud services enable drive.googleapis.com --project=jescri-migracao` (confirmada habilitada e assim permanece).
   - **Achado adicional, comprovado via Execution Log (`clasp logs`)**: mesmo depois da API habilitada, o runtime do Apps Script continuou tentando (e falhando) auto-habilitá-la em tempo de execução (`ServiceUsage.BatchEnableServices`, severidade ERROR, `2026-07-06T00:59:17Z` e `2026-07-06T01:12:55Z`) — e não há nenhum log de execução de `iniciarEnvioResumable`/`finalizarEnvioResumable` depois disso. Isso indica que a **autorização OAuth do deploy** (`executeAs: USER_DEPLOYING`, conta `elafashionmkt@gmail.com`) foi concedida quando a API ainda não estava habilitada e ficou presa nesse estado — habilitar a API depois não atualiza retroativamente uma concessão já existente. Sintoma no navegador: "Failed to fetch" (erro de rede do lado do cliente, sem chegar a logar nada no backend), consistente com a autorização do Web App estar quebrada, não com CORS na chamada direta ao Drive.
   - **Ação pendente, não executável por CLI/gcloud/clasp**: o usuário/dono do script precisa reautorizar manualmente pela UI do Apps Script (abrir o projeto em script.google.com, rodar qualquer função pelo editor uma vez, aceitar o diálogo de permissões se aparecer) para que a concessão reflita a API já habilitada.
   - **Lição**: um escopo OAuth novo no manifesto exige confirmar que a API correspondente está habilitada no projeto GCP vinculado **antes** da primeira autorização — habilitar depois não é suficiente sozinho, precisa de uma reautorização explícita.
   - **Impacto na automação de pasta no Drive (2026-07-06)**: `gerarNovoMesCompleto()` (`mae/Código.js`) e `obterOuCriarPastaDestino()` (`mae/WebApp.js`) agora criam a pasta da influenciadora automaticamente (no primeiro briefing e no primeiro upload, respectivamente — ver `FLOW.md`). Essa automação depende do mesmo escopo `drive` afetado pela reautorização pendente acima — só é testável de ponta a ponta depois que o usuário reautorizar manualmente.
8. **Reversão de arquitetura (2026-07-06, pedido do usuário)**: a pasta da influenciadora no Drive voltou a ser rastreada por `BASE DE DADOS.PASTA_DRIVE_LINK` (coluna, resolvida por `getHeaderMap()`) em vez de `PropertiesService` por cupom — decisão anterior (documentada nesta sessão de auditoria) revertida porque a equipe precisa ver/editar o link direto na planilha. Função única para os dois gatilhos (upload e primeiro briefing): `obterOuCriarPastaInfluenciadoraPorLinha()` (`mae/WebApp.js`). Migração: pastas já criadas via `PropertiesService` antes desta mudança são reaproveitadas (nunca duplicadas) na primeira vez que qualquer um dos dois gatilhos rodar para aquela influenciadora — `getIdPastaDriveCupom()` foi mantida só para essa leitura de migração.
...
- **(2026-07-06) Deploy `@31` a partir de branch NÃO mergeada — desvio da convenção abaixo, autorizado explicitamente pelo usuário**: `clasp push` + `clasp deploy -i` rodados a partir do worktree da branch `worktree-glistening-bouncing-garden` (PR #13, ainda aberto/draft), não de `main`. Deployment pública agora reflete o conteúdo do PR #13 (cadastro nativo no Portal, nome=`INFLU_KEY`, status `AGUARDANDO`, pasta Drive via `PASTA_DRIVE_LINK`, fix de inversão de data) — **`main` e a deployment pública estão temporariamente dessincronizados** até o PR #13 ser mergeado. Se outro agente/sessão rodar `clasp push` a partir de `main` antes do merge, reverte esses fixes na deployment pública sem aviso. Resolver mergeando o PR #13 o quanto antes, ou repetindo `clasp deploy -i` a partir de `main` pós-merge (mesmo conteúdo, sem efeito adicional).
