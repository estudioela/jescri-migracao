...
- **(PENDENTE ação manual do usuário, 2026-07-06) Drive API estava desabilitada no projeto GCP vinculado — habilitação sozinha não bastou**: `mae/appsscript.json` declara o escopo `https://www.googleapis.com/auth/drive`, mas `drive.googleapis.com` nunca tinha sido habilitada no projeto GCP standard vinculado (`jescri-migracao`, nº `607782229022`). Habilitada via `gcloud services enable drive.googleapis.com --project=jescri-migracao` — permanece habilitada. **Mas o Execution Log (`clasp logs`) mostra que a autorização OAuth do deploy (`executeAs: USER_DEPLOYING`) ficou presa no estado anterior à API habilitada** — o runtime continuou tentando e falhando auto-habilitar a API em tempo de execução, e não há mais nenhum log de `iniciarEnvioResumable`/`finalizarEnvioResumable` depois disso (sintoma no navegador: "Failed to fetch", sem nada no backend). **Precisa reautorização manual pela UI do Apps Script** (abrir o projeto em script.google.com, rodar qualquer função pelo editor, aceitar o diálogo de permissões) — não é replicável por `clasp`/`gcloud`. Detalhe completo: `SYSTEM_TRUTH.md` seção 5, item 7. **Lição para o futuro**: qualquer novo escopo OAuth em `appsscript.json` exige confirmar que a API correspondente está habilitada no projeto GCP vinculado **antes** da primeira autorização — habilitar depois não corrige uma concessão já existente sozinho.
