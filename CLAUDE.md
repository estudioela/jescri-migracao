...
- **(Resolvido em 2026-07-06) Drive API desabilitada no projeto GCP vinculado**: `mae/appsscript.json` declara o escopo `https://www.googleapis.com/auth/drive`, mas `drive.googleapis.com` nunca tinha sido habilitada no projeto GCP standard vinculado (`jescri-migracao`, nº `607782229022`). Quebrou upload de matérias (dependência direta) e histórico (indireto — falha de autorização de um escopo declarado pode invalidar a execução inteira do Web App). Fix: `gcloud services enable drive.googleapis.com --project=jescri-migracao` (conta `elafashionmkt@gmail.com` tinha `serviceusage.googleapis.com` habilitada, permissão suficiente, sem bloqueio de organização). Nenhuma mudança em código/manifest/deploy foi necessária. Detalhe completo: `SYSTEM_TRUTH.md` seção 5, item 7. **Lição para o futuro**: qualquer novo escopo OAuth em `appsscript.json` exige confirmar que a API correspondente está habilitada no projeto GCP vinculado, antes de depender dele em produção.
